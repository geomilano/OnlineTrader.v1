...
function orderDetailListener(titanium_bridge){
	Ti.App.addEventListener(titanium_bridge, function(e) {
		if(titanium_bridge == "app:getPendingList"){
			getStateSelector(e,'');
			//return true;
		}else if(titanium_bridge == "app:PosParam"){
...
function getStateSelector(e,seltype){
	//alert( 'web =' +localStorage.getItem("stateURL"));
...
function getProductelector(e){
	$.getJSON( e.product+e.session, function( data ) {
	    ...
	    if(data.status == 'error'){
	    	...
	    	var item = '<select name="sel_product" class="selector" id="sel_product" style="width:100%;">';
